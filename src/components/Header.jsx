  ...
  return (
    ...
    py-6 sticky top-0 z-30 border-[8px] border-[#a39446]'>
      ...
      <div className={`${isMenuOpen ? 'flex' : 'hidden'} w-full h-fit bg-slate-800
        ...
        {navItems.map(({ link, path }) => (
          <Link key={path} className='text-white uppercase font-semibold
          cursor-pointer p-3 rounded-lg hover:bg-gray-400 hover:text-black
        ...
        <button className='bg-gray-400 px-8 py-3 rounded-full hover:bg-white
        hover:text-black font-bold mt-3 hidden lg:flex transform hover:scale-110
        transition-transform duration-300 cursor-pointer'>BOOK NOW</button>
